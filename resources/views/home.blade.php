        ...
        <div class="row">
          <div class="col-md-6">
            <div class="navbar-header">
              <!-- Branding Image -->
              @if($setting)
                <a class="tt"   href="{{url('/')}}"><h4 class="heading">{{$setting->welcome_txt}}</h4></a>
              @endif
            </div>
          </div>
          <div class="col-md-6">
            <div class="collapse navbar-collapse" id="app-navbar-collapse">
              ...
              <ul class="nav navbar-nav navbar-right">
                ...
                @guest
                  <li><a href="{{ route('login') }}" title="Giriş">Giriş</a></li>
                  <li><a href="{{ route('register') }}" title="Kayıt">Kayıt</a></li>
                @else
                  <li class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                      @if ($auth->role == 'A')
                        <li><a href="{{url('/admin')}}" title="Yönetici">Yönetici Paneli</a></li>
                      @elseif ($auth->role == 'S')
                        <li><a href="{{url('/admin/my_reports')}}" title="Panel">Panel</a></li>
                      @endif
                      <li>
                        <a href="{{ route('logout') }}"
                    ...
                    <div class="row">
                      <div class="col-xs-6 pad-0">
                        <ul class="topic-detail">
                          <li>Soru İşareti Başına <i class="fa fa-long-arrow-right"></i></li>
                          <li>Toplam puan <i class="fa fa-long-arrow-right"></i></li>
                          <li>Toplam sorular <i class="fa fa-long-arrow-right"></i></li>
                          <li>Toplam zaman <i class="fa fa-long-arrow-right"></i></li>
                          <li>Sınav Fiyatı <i class="fa fa-long-arrow-right"></i></li>
                        </ul>
                      </div>
                      <div class="col-xs-6">
               ...
               <div class="card-action text-center">
                  ...
                  @endif
                    ...
                    @if($auth->topic()->where('topic_id', $topic->id)->exists())
                      <a href="{{route('start_quiz', ['id' => $topic->id])}}" class="btn btn-block" title="Testi Başlat">Testi Başlat </a>
                    @else
                      {!! Form::open(['method' => 'POST', 'action' => 'PaypalController@paypal_post']) !!} 
                        ...
                        <button type="submit" class="btn btn-default">Öde  <i class="{{$setting->currency_symbol}}"></i>{{$topic->amount}}</button>
                          @else 
                ...
                {{--   <div class="card-action">
                    ...
                    @endphp
                    <a href="{{$a ? url('start_quiz/'.$topic->id.'/finish') : route('start_quiz', ['id' => $topic->id])}}" class="btn btn-block" title="Testi Başlat">Testi Başlat
                    </a>
                  </div> --}}
                </div>
              </div>
    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="home-main-block">
              ...
              @endif
                <blockquote>
                  Lütfen <a href="{{ route('login') }}">Giriş Yap</a>   Testi Başlat >>>
                </blockquote>
            </div>
        </div>
    </div>
